...
| **Current sprint** | Sprint 4 — Write-up and defense |
| **Current task** | `4.2.1` — finalise `README.md`, verify a fresh clone reproduces the documented setup |
| **Recently merged** | Sprints 0–2 complete; Sprint 3.1, 3.2, 3.4 complete; Sprint 4.1 complete as of 2026-08-13; 3.3 deferred |
| **Status** | 🟢 Sprint 0, 1 and 2 all done. Sprint 3.1 (dashboard) and 3.2 (interstitial) code-complete, real-browser manual verification still pending (same category as D7/D8). **3.3 (deployment) deferred by Hammad** — needs external accounts only he can provision. 3.4 (E2E validation) run against the local stack instead of a deployed one: 20/30 correct, 1 deep-path false positive — an honest, root-caused shortfall, not chased into a retrain. **4.1 (evaluation report / thesis update) complete**: every ⟨M-nn⟩ placeholder across all thesis chapters replaced with real measured numbers, `docs/thesis/06-conclusion.md` rewritten to reflect the finished project instead of the mid-project draft, `LIMITATIONS.md` written, `ml/scripts/sensitivity.py` added to close the one measurement (fusion-weight sensitivity) that had never actually been run. Moving to 4.2 (documentation) — demo video (4.3.1) and live deployment remain out of scope until 3.3 is unblocked |
| **Blocked?** | No |
...
| **2026-08-13** | **Claude** | **Sprint 3.2 complete: phishing interstitial.** New `extension/modules/interstitial.js`, a default-world content script rendering the warning inside a **closed Shadow DOM** — the page under assessment is untrusted, so it shouldn't be able to inspect or interfere with the warning shown about it. `background.js` triggers it via `chrome.tabs.sendMessage` exactly when `result.verdict === "phishing"`, gating on the label rather than duplicating the 0.70 threshold in extension code. "Leave" round-trips through a new `INTERSTITIAL_LEAVE` message since content scripts have no direct `chrome.tabs` access; "Continue" just removes the DOM node — nothing persisted, so a fresh navigation re-warns by construction, not by an extra check. No automated test: DOM/Shadow-DOM injection isn't meaningfully testable in the existing Node-VM-sandbox harness without adding jsdom as a new dependency for one test, so this follows the same precedent as D7/D8 — `tests/manual/interstitial_test.md`, real-browser verification still pending. |
| **2026-08-13** | **Hammad** | Asked how to handle Sprint 3.3 (live deployment — needs Railway/Render/Vercel accounts); answered **"we will do these last steps later."** Deployment deferred, not abandoned; config (`extension/config.js`, `NEXT_PUBLIC_BACKEND_URL`) is ready for whenever accounts are provisioned. |
| **2026-08-13** | **Claude** | **Sprint 3.4 (E2E validation) executed against the local stack**, decoupled from 3.3 since the detection-accuracy claim doesn't depend on hosting location. `tests/e2e/run_system_test.py` POSTed 30 real URLs (15 live OpenPhish — deliberately not PhishTank, to avoid training-source overlap; 15 legitimate weighted toward deep paths, D1's original blind spot) at the running local Docker backend's `/analyze`, exercising the full live pipeline including real VirusTotal lookups. **Result: 20/30 correct, 1 deep-path false positive** (`docs.python.org/3/library/asyncio.html` scored 71%) — below the 26/30 acceptance bar. Root-caused via SHAP rather than dismissed: the false positive is driven by `num_digits` being a raw count (a single incidental digit — the `3` in a Python version path — carries +0.87 log-odds impact) and by `url_length` carrying a strong negative learned weight that under-scores phishing URLs on trusted free-hosting platforms (`vercel.app`, `typedream.app`, `pages.dev`). This is the same lexical-feature brittleness already disclosed in Sprint 1's `training_log.md` (`url_entropy`-driven, 8.8% FP rate on the 1,488-URL holdout) reproducing on live data, not a new defect. **Decided not to retrain**: doing so on an n=30 signal from a single digit collision would be statistically unsound and repeat the exact overfit-to-the-report pattern this project rejected at the corpus rebuild, the FP-holdout measurement, and the faithfulness check. Recorded honestly in `ml/reports/e2e_validation.md` and `ROADMAP.md` §3.4, alongside the already-disclosed faithfulness (87.5%/90%) and FP-holdout (8.5%) shortfalls — a fourth data point supporting the same "lexical URL features alone are not sufficient for a robust production deployment" limitation, going into `LIMITATIONS.md` in Sprint 4. |
| **2026-08-13** | **Claude** | **Sprint 4.1 complete: thesis updated with real measured numbers throughout, plus one genuinely new measurement.** Filled every ⟨M-nn⟩ placeholder across `docs/thesis/*.md` from the reports already sitting in `ml/reports/` (leakage audit before/after, training log, evaluation report, fusion weights) and the freshly-run `tests/e2e/run_system_test.py`: dataset audit tables (5.2–5.5), corpus composition (5.11), detection performance under all three splits with the currently-deployed model re-evaluated on its own reproducible split rather than retrained (5.12–5.13, confirmed via matching `model_sha256`), baseline comparison with an honest no-B5-row explanation (5.14), FP holdout (5.15), calibration with the reliability diagram copied into `docs/thesis/diagrams/out/` (5.16), faithfulness (5.17), latency (5.19), and the 30-URL E2E result with full root-cause analysis (5.21–5.22). One placeholder (fusion-weight sensitivity, M-15) had never actually been measured — no `sensitivity.py` existed — so it was built: perturbs each fusion weight against a fixed synthetic "typical page" browser-signal profile applied uniformly across the 3,937-URL temporal-split test set (chosen because no corpus pairs real per-URL browser telemetry with a label, and fabricating one would repeat the exact error ADR-014 already rejects), measuring verdict-band churn and F1 movement. First draft's own written interpretation ("no perturbation changes F1 by more than a few thousandths") was checked against its own output and found wrong — real F1 deltas were low-hundredths and doubling the weights moved 43.9% of URLs across a verdict band — so the interpretive text was corrected to match the actual numbers rather than left as a plausible-sounding but false claim, and the report section regenerated. Rewrote `docs/thesis/06-conclusion.md` in full: objectives O4/O5/O7 and claims C1/C2/C3 were still written as "instrumented but unmeasured" from mid-project; updated to reflect real outcomes including the two that fell short of their target (faithfulness 87.0%/90%, E2E 20/30) rather than hiding the shortfall inside a checkmark. Updated `04-implementation.md` §4.4.3/§4.7.7 (D7/D8 from present-tense "must be fixed" to past-tense "was fixed, real-browser confirmation pending") and `01-introduction.md`'s claim statements (stale section cross-references, C3 updated with the real 87.0% figure). Wrote `LIMITATIONS.md` at the repo root. Caught and fixed one internal defect while doing this: two different tables in `05-testing.md` were both numbered "Table 5.22" — renumbered the later one to 5.23. `pytest` (55 passed), `npm test` (both JS suites) and `ruff check` all still green after the `sensitivity.py` addition and `ruff.toml` per-file-ignore. **Not done in this pass:** rebuilding `docs/thesis/out/thesis.docx` — neither `pandoc` nor a JDK is available in this environment, so the Markdown sources are updated and correct but the committed `.docx` is stale until `docs/thesis/build.sh` is run on a machine that has them. |
